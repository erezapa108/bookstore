...
function Home() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      void fetchBooks(1, debouncedSearchTerm).catch(() => {});
    }, 0);

    return () => window.clearTimeout(timeoutId);
  }, [debouncedSearchTerm, fetchBooks, searchTerm]);
  ...
  return (
    <div ref={formRef} style={{ padding: "20px" }}>
      <h1 style={{ color: "#2F6F5E" }}>Bookzone.id Admin</h1>
      <p style={{ color: "#3D3A34" }}>
        Total {shownCount}/{total} buku
      </p>
